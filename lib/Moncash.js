...
const Prefunded		= require('./resources/Prefunded');
...
class Moncash {
	...
	configure(config){
		...
		this.customer 	= new Customer(this.config,this._request);
		this.prefunded 	= new Prefunded(this.config,this._request);
	...
	/**
	 * Absolute path to the official "Pay with MonCash" button image shipped with the SDK.
	 * @returns {string} Path to assets/moncash-button.png
	 */
	static getButtonPath() {
		return path.join(__dirname, '..', 'assets', 'moncash-button.png');
	}
}
